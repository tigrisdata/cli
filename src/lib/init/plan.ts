...
export function emitGettingStarted() {
  const plan = {
    ...
    nextAction: {
      type: 'agent_action',
      prerequisite: 'https://github.com/tigrisdata/skills',
      steps: [
        {
          id: 'select_org',
          description:
          ...
          command: 'npx -y @tigrisdata/cli orgs list --format json',
        },
        {
          id: 'create_bucket',
          description:
            'Create a bucket for this project, or reuse an existing one from `npx -y @tigrisdata/cli buckets list --format json`. Choose a clear, project-specific name.',
          command:
            'npx -y @tigrisdata/cli buckets create <bucket> --format json',
        },
        {
          ...
          command:
            'npx -y @tigrisdata/cli access-keys create <key-name> --format json',
        },
        {
          id: 'assign_role',
          description:
          ...
          command:
            'npx -y @tigrisdata/cli access-keys assign <key-id> --bucket <bucket> --role Editor --format json',
        },
        {
          id: 'write_env',
          description:
